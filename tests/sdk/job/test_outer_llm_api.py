...
from protollm.sdk.sdk.sdk.models.job_context_models import PromptModel, ResponseModel, ChatCompletionModel, \
...
@pytest.fixture
def mock_openai():
    with patch('sdk.sdk.job_context.outer_llm_api.OpenAI') as mock_openai:
...
def test_outer_llmapi_initialization():
    ...
    assert len(outer_llm_api.client.api_key) == len("sk-or-vv-c49f40fdb086053ec32c6ae2723b8d222cb7767f3b98527e7ae282986e7d33ed")
...
def test_outer_llmapi_inference_server_error(mock_openai, outer_llm_api):
    ...
    with pytest.raises(Exception, match="The response generation has been interrupted. Error: The LLM server is not available."):
