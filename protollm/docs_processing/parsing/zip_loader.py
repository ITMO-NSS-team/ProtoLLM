...
from protollm.docs_processing.parsing.parsers import ParsingScheme, DocType, BaseParser, PDFParser, WordDocumentParser
    ...
    def __init__(
        ...
        if doc_type is not DocType.zip:
            if doc_type is DocType.unsupported:
                ...
                raise ValueError(f'The {doc_type} file type does not match the Loader! Use a suitable one.')
        self.byte_content = byte_content
        self._logger = parsing_logger or ParsingLogger(silent_errors=silent_errors, name=__name__)
        self.pdf_parser = PDFParser(pdf_parsing_scheme, pdf_extract_images, pdf_extract_tables, pdf_parse_formulas,
                                    pdf_remove_service_info)
        self.word_doc_parser = WordDocumentParser(word_doc_parsing_scheme, word_doc_extract_images,
                                                  word_doc_extract_tables, word_doc_parse_formulas,
                ...
                match doc_type:
                    ...
                    case _:
                        if Path(file_name).suffix:
                            self._logger.info(f"Skip file processing in zip, no suitable parser for {path}")
                        continue

                self._logger.info(f"Processing file in zip: {path}")
                blob = Blob.from_data(z.open(info).read(), path=path, mime_type=doc_type.value)
