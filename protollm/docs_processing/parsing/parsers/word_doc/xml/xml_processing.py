...
from protollm.docs_processing.parsing.parsers.word_doc.xml.xml_tag import XMLTag
from protollm.docs_processing.parsing.parsers.word_doc.docx_parsing_config import DocxParsingConfig


def _convert_to_latex(xml_element: etree.Element,
...
def _extract_image_data(xml_element: etree.Element,
                        parsing_config: DocxParsingConfig
                        ) -> dict:
    ...
    return {'image_filename': image_element.filename,
            'image': image}


def _parse_raw_xml_element(raw_xml_element: etree.Element,
                           parsing_config: DocxParsingConfig
                           ) -> tuple[list[str], dict]:
    ...
    for child_element in raw_xml_element:
        match child_element.tag:
            ...
            case XMLTag.raw:
                child_texts, child_extracted_image_data = _parse_raw_xml_element(child_element, parsing_config)
            ...
            case XMLTag.image:
                if parsing_config.extract_images:
                    ...
                    extracted_image_data[image_name] = _extract_image_data(child_element, parsing_config)

    return texts, extracted_image_data


def process_paragraph_body(paragraph: Paragraph,
                           parsing_config: DocxParsingConfig
    ...
    for element in xml_paragraph:
        match element.tag:
            case XMLTag.raw:
                extracted_texts, extracted_image_data = _parse_raw_xml_element(element, parsing_config)
            ...
            case XMLTag.math | XMLTag.math_paragraph:
                if parsing_config.parse_formulas:
                    ...
                    extracted_data['formulas'][formula_name] = _convert_to_latex(element, parsing_config)
