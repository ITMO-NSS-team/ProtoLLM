...
from protollm.docs_processing.parsing.parsers.utilities import is_bulleted_text
from protollm.docs_processing.parsing.parsers.word_doc.xml import process_paragraph_body
from protollm.docs_processing.parsing.parsers.word_doc.docx_parsing_config import DocxParsingConfig
...
def _get_metadata(paragraph: Optional[Paragraph] = None) -> dict:
    ...
    if paragraph is not None:
        ...
        font_size = paragraph.runs[0].font.size or font_size if paragraph.runs else font_size
        ...
        is_bullet_list = is_bulleted_text(paragraph.text) | ("<w:numPr>" in paragraph._element.xml)
        ...
        first_line_indent = paragraph_first_line_indent.emu if paragraph_first_line_indent is not None \
...
def _process_paragraph(paragraph: Paragraph,
                       parsing_config: DocxParsingConfig
                       ) -> tuple[str, dict]:
    paragraph_text, paragraph_metadata = process_paragraph_body(paragraph, parsing_config=parsing_config)
...
def _convert_to_html(table: Table,
                     parsing_config: DocxParsingConfig,
                     is_nested: bool = False) -> tuple[str, list[dict]]:
    cells_metadata = []

    def iter_cell_block_items(cell: _Cell) -> Iterator[str]:
        for block_item in cell.iter_inner_content():
            if isinstance(block_item, Table):
                inner_html_table, inner_cells_metadata = _convert_to_html(block_item, parsing_config, is_nested=True)
                cells_metadata.extend(inner_cells_metadata)
                yield inner_html_table
            elif isinstance(block_item, Paragraph):
                paragraph_text, metadata = _process_paragraph(block_item, parsing_config)
...
def _process_table(table: Table,
                   parsing_config: DocxParsingConfig) -> tuple[str, dict]:
...
def parse_docx_to_lines(stream,
    ...
    parsing_config = DocxParsingConfig(document, extract_images, parse_formulas)
