...
from protollm.docs_processing.parsing.parsers.utilities import HEADING_KEYWORDS, HEADING_STOP_LIST
...
def add_headings_hierarchy(lines: list[str], metadata: list[dict]) -> tuple[list[str], list[dict]]:
...
def get_chapters(lines: list[str], metadata: list[dict]) -> tuple[list[str], list[dict]]:
    chapters, meta = [], []
    cur_chapter = ''
    for line, line_meta in zip(lines, metadata):
...
def get_paragraphs(lines: list[str], metadata: list[dict]) -> tuple[list[str], list[dict]]:
    paragraphs, meta = [], []
    cur_paragraph = ''
    for line, line_meta in zip(lines, metadata):
        if paragraphs and line_meta['is_bullet_list'] and (cur_paragraph == line_meta['headings'][-1]):
