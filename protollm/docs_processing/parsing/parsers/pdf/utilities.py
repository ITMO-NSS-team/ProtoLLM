...
from protollm.docs_processing.parsing.parsers.utilities import HEADING_KEYWORDS, HEADING_STOP_LIST, TITLE_KEYWORDS, \
    FOOTER_KEYWORDS
from protollm.docs_processing.parsing.exceptions import (NoTextLayerError, ParseImageWarning, TitleExtractingWarning,
                                                PageNumbersExtractingWarning, FooterExtractingWarning)
...
def convert_table_to_html(table) -> str:
    ...
    :return:
    """
    processed_table = [[item.replace('\n', ' ') if item is not None else '' for item in row] for row in table]
...
def get_heading_info(element_info, heading_env, doc_info):
    ...
    if is_bold:  # if the line starts with the numeric prefix and is bold - it is heading of the particular level
...
def get_numeric_prefix_str(text_line):
    ...
    numeric_pref = re.match('^([0-9]+(\.)?)+', line_text)  # check if the line starts with the numeric prefix
    if numeric_pref is not None:
    ...
    for page_number, page in enumerate(pages_layout):
        ...
        for element in page:
            if isinstance(element, LTTextContainer):
                if not isinstance(element, LTTextLine):  # text element is a Box and should be unpacked to Lines
                    for line in element:
                        text_lines_stat.append(len(line.get_text().split(' '))) # calculate number of words in parsed line
                else:
                    text_lines_stat.append(len(element.get_text().split(' '))) # calculate number of words in parsed line
                ...
                for line in text_lines_lst:
                    if page_tables is not None:  # if there are any tables on the page
                        if is_element_inside_any_table(line, page, page_tables):
                            table_id_found = find_table_for_element(line, page, page_tables)
                            if table_id_found is not None:  # text element is a part of the table
                                page_structure.append(
                                    {'element': line, 'meta': {'type': 'table', 'id': table_id_found}})
                                continue
                    # text line is not a part of the table
                    page_structure.append({'element': line, 'meta': {'type': 'text', 'id': -1}})
            elif isinstance(element, LTFigure):  # element is an image
                page_structure.append({'element': element, 'meta': {'type': 'image', 'id': -1}})
...
def get_document_formatting(pages_structure):
    ...
    for page in pages_structure:
        page_structure = []
        prev_line_bottom_border = None  # y coordinate of the previous line's bottom border
        ...
        for element_info in page:
            ...
            if element_info['meta']['type'] != 'image' and element_info['element'].get_text() != '':
                element = element_info['element']
                if prev_line_bottom_border is not None:
                    elem_line_spacing = prev_line_bottom_border - element.y0  # space between the lines
                else:
                    elem_line_spacing = -1  # first line on the page

                prev_line_bottom_border = element.y1  # update previous line bottom border attribute
                ...
                for character in element:
                    ...
                    if elem_start_symbol == '':
                        elem_start_symbol = 'letter' if character.get_text()[0].isalpha() else \
                            ('digit' if character.get_text()[0].isdigit() else 'symbol')
                    if isinstance(character, LTChar):
                ...
                doc_line_spacing_counter[elem_line_spacing] += 1  # add line spacing attribute
                doc_left_margin_counter[elem_left_margin] += 1  # add left margin attribute
                doc_font_size_counter[elem_font_size] += 1  # add font size attribute
                doc_main_font_counter[elem_font_name] += 1  # add font name info attribute

            element_info['meta']['format'] = element_format_info  # set format info for the element
            page_structure.append(element_info)
        doc_structure.append(page_structure)

    doc_font_size = doc_font_size_counter.most_common(1)[0][0]  # main document's font size
    doc_line_spacing = doc_line_spacing_counter.most_common(1)[0][0]  # main document's line spacing
    doc_left_margins = [x[0] for x in doc_left_margin_counter.most_common(2)]  # two most common left margins
...
def parse_pdf_to_lines(stream,
                       parse_images=False,
                       parse_tables=True,
                       parse_formulas=False,
                       remove_service_info=False) -> tuple[list[str], list[dict]]:
    """
    ...
    params = LAParams(line_overlap=0.5,
                      char_margin=15.0,
                      line_margin=0.1,
                      word_margin=2.0,
                      boxes_flow=1.0,
    ...
    for page_layout in pages_layout:
        for element_info in page_layout:
            if element_info['meta']['type'] == 'text' or element_info['meta']['type'] == 'table':
                is_text_in_doc = True
                break
        if is_text_in_doc:
            break
    ...
    heading_env = -1  # level of the current heading's environment (-1 if not the heading's environment)
    heading_lst = []
    ...
    for page_number, page in enumerate(doc_structure):
        ...
        if parse_images:
            try:
                ...
                warnings.warn('Error in pages structure formatting', category=ParseImageWarning)
                page_object = None
        ...
        for element_info in page:
                ...
                if heading_lvl != -1:  # text line is a part of some heading
                    ...
                    if heading_env != -1:  # previous element contained heading
                        ...
                        if start_symbol != 'symbol':
                            if element_spacing > 1.1 * doc_info['line_spacing'] or \
                                    element_margin > doc_info['left_margin'][0]:
                                paragraph_id += 1  # new paragraph
            ...
            if element_text != '' and (is_heading != 0 or paragraph != -1):
                ...
                document_meta[-1].append({'type': element_meta['type'],
        ...
        try:
            ...
            for text_line in potential_title_page:
                ...
                if is_title:
                    break
            if not is_title:
                # for title_ending_key in TITLE_KEYWORDS:
                #     if title_ending_key in potential_title_page[-1].lower():
                #         is_title = True
                #         break
            ...
            warnings.warn('Can not skip title-related service information due to unknown title formatting',
                ...
                if len(page) > 0:
                    ...
                    for char in page[0]:  # number is at the beginning of the page
                        if not char.isdigit():
                            only_digits = False
                    if only_digits:
                        document_content[page_number] = document_content[page_number][1:]
