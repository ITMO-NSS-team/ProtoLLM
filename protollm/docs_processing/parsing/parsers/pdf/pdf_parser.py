...
from protollm.docs_processing.parsing.parsers.utilities import CONTENTS_KEYWORDS
from protollm.docs_processing.parsing.utilities import correct_path_encoding, is_bad_encoding
from protollm.docs_processing.parsing.exceptions import EncodingError, ChaptersExtractingFailedWarning
...
class PDFParser(BaseParser):
    ...
    def __init__(self,
        ...
        try:
            import docs_processing.parsing.parsers.pdf.utilities
    ...
    def lazy_parse(self, blob: Blob) -> Iterator[Document]:
        from docs_processing.parsing.parsers.pdf.utilities import parse_pdf_to_lines
        ...
        with blob.as_bytes_io() as pdf_file_obj:
            # TODO: parse_formulas support
            lines, metadata = parse_pdf_to_lines(pdf_file_obj,
        ...
        if max_hierarchy_lvl != 0:
            ...
            for text, meta in zip(lines, metadata):
                if self.remove_service_info:
                    if len(meta['headings']) > 0 and meta['headings'][0].lower() in CONTENTS_KEYWORDS:  # len(meta['headings']) == 0 or
                        continue
        ...
        match self.parsing_scheme:
            ...
            case ParsingScheme.full:
                ...
                meta = {'page': 'all', 'source': source, 'file_name': file_name,
            ...
            case ParsingScheme.chapters:
                if not is_heading_extracting_correct:
                    warnings.warn('Can not correctly extract chapters due to complex document structure',
                                  category=ChaptersExtractingFailedWarning)
                text_lst = []
                heading = ''
                for text, meta in zip(lines, metadata):
                    if len(meta['headings']) != 0:  # text is a part of some chapter
                        if meta['headings'][0] == heading:  # it has the same chapter's heading
                            text_lst.append(text)  # add element's text to the document's content
                        elif heading != '':  # other heading
                            document_text = ' '.join(text_lst)
                            document_meta = {'heading': heading, 'source': source, 'file_name': file_name,
                                             'is_heading_extracting_correct': is_heading_extracting_correct}
            ...
            case ParsingScheme.paragraphs:
                text_lst = []
                paragraph = -1
                document_meta = {'source': source, 'file_name': file_name,
                                 'is_heading_extracting_correct': is_heading_extracting_correct}
                for text, meta in zip(lines, metadata):
                    if len(meta['headings']) != 0:  # text is a part of some chapter
                        if meta['paragraph'] != -1:  # text is a part of some paragraph (not header)
                            if meta['paragraph'] == paragraph:  # it has the same paragraph's number
                                text_lst.append(text)  # add element's text to the document's content
                                document_meta = {**meta, 'source': source, 'file_name': file_name,
                                                 'is_heading_extracting_correct': is_heading_extracting_correct}
