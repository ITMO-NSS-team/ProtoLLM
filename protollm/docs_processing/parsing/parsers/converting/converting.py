...
from protollm.docs_processing.parsing.exceptions import ConvertingError
...
def _convert_with_soffice(
    ...
    expected_path = Path(output_directory, '.'.join((Path(filename).stem, target_doc_type)))
    try:
        ...
        if error and not os.path.isfile(expected_path):
            raise ConvertingError(f"Could not convert file to {target_doc_type}\n{error}")
