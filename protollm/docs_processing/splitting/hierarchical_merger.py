...
def _get_metadata(metadata: dict[str, Any]) -> dict[str, Any]:
    # TODO: add other information, i.e. about urls, images, tables
    chapter = metadata['headings'][0] if metadata.get('headings', []) else ''
    source = metadata.get('source', '')
    meta = {'chapter': chapter, 'source': source, 'file_name': metadata.get('file_name', ''),
            'keywords': metadata.get('keywords', []), 'object': [metadata.get('object', '')],
            'action': [metadata.get('action', '')]}
    return meta


class HierarchicalMerger(TextSplitter):
    ...
    def _merge_documents(self, documents: Iterable[Document]) -> List[Document]:
        ...
        for doc in documents[1:]:
            ...
            if doc_chapter == chapter and doc_source == source:  # document is related to the same chapter
                merged_text = self.joiner.join((merged_chunk_content, doc_text))
                if self._length_function(merged_text) <= self._chunk_size:
                    # add current document's text to the merged chunk
                    # TODO: add other information, i.e. about urls, images, tables from doc_meta to merged_chunk_meta
            ...
            if merged_chunk_content:
                merged_chunk_meta['keywords'] = ', '.join(set(merged_chunk_meta['keywords']))
                merged_chunk_meta['object'] = ', '.join(set(merged_chunk_meta['object']))
                merged_chunk_meta['action'] = ', '.join(set(merged_chunk_meta['action']))
                transformed_docs.append(self._create_document(merged_chunk_content, merged_chunk_meta))
    ...
    def _create_document(self, text: str, metadata: dict[str, Any]) -> Document:
        ...
        if text_len > self._chunk_size:
            logger.warning(f"A chunk of size {text_len} was encountered, "
    ...
    def split_text(self, text: str) -> List[str]:
        raise NotImplementedError('split_text is not implemented in HierarchicalMerger')

    def _merge_splits(self, splits: Iterable[str], separator: str) -> List[str]:
        raise NotImplementedError('_merge_splits is not implemented in HierarchicalMerger')
