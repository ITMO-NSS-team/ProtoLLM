...
class MultiMetadataAppender(TextSplitter):
    ...
    def _split_with_additional_metadata(self, documents: Iterable[Document]) -> List[Document]:
        texts, metadatas = [], []
        for doc in documents:
            doc_metadata = doc.metadata
            doc_metadata['object'], doc_metadata['action'] = (
                self.keyword_extractor.get_object_action_pair(doc.page_content))
            doc_metadata['keywords'] = self.keyword_extractor.get_keywords(doc.page_content)
